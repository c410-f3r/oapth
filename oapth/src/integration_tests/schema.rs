...
pub(crate) async fn migrate_works<B>(buffer: &mut String, c: &mut Commands<B>, aux: AuxTestParams, oapth_schema_tables: usize)
where
  B: Backend
{
  ...
  assert_eq!(
    c.backend.tables(aux.default_schema).await.unwrap().len(),
    4 + aux.schema_regulator
  );
  assert_eq!(c.backend.tables(aux.oapth_schema).await.unwrap().len(), oapth_schema_tables);
}
