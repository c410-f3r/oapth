  ...
  B: crate::BackEnd,
{
  ...
  crate::integration_tests::_create_foo_table(c, "bar.").await;
  assert_eq!(c.back_end.all_tables("bar").await.unwrap().len(), 1);

  c.clean().await.unwrap();

  assert_eq!(c.back_end.all_tables(aux.default_schema).await.unwrap().len(), 0);
  assert_eq!(c.back_end.all_tables("bar").await.unwrap().len(), 0);
}
