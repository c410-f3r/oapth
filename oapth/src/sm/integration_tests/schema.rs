...
use crate::sm::{integration_tests::AuxTestParams, Commands, MigrationGroup, SchemaManagement};
use std::path::Path;
  ...
  D: SchemaManagement,
{
  ...
  c.database.migrations(buffer_cmd, &initial, &mut db_migrations).await.unwrap();
  ...
  db_migrations.clear();
  c.database.migrations(buffer_cmd, &more_stuff, &mut db_migrations).await.unwrap();
  ...
  c.database.table_names(buffer_cmd, &mut idents, aux.default_schema).await.unwrap();
  assert_eq!(idents.len(), 4 + aux.schema_regulator);
  idents.clear();
  c.database.table_names(buffer_cmd, &mut idents, aux.oapth_schema).await.unwrap();
  assert_eq!(idents.len(), oapth_schema_tables);
}
